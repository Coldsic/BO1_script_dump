...
mortarTeamSpawn(spawners, node, mortar_targets)
{
	...
	index = 0;
	for(;;) // done this way so it can loop in case of a failed spawn
	{
...
aimGuyMortarsUntilDeath(node)
{
	...
	node notify ("stopIdle");
	// node anim_reach(aimGuy, "pickup_alone"); JAS May 13 2010, shouldn't need this, happened in aimguy()
...
dualMortarUntilDeath(node)
{
	...
	loadGuy = self.loadGuy;
	aimGuy = self.aimGuy;
	guy = self.guy; // array of two guys, actually
	...
	node notify ("stopIdle"); // in case we broke abruptly from a previous loop to start this one
	//node anim_reach (guy, "pickup"); // JAS shouldn't need this, happened in aimguy() or loadguy()
...
//	self thread debugOrigin();
	...
	node anim_reach(self, "pickup"); // JAS, get guys into position right away
	
	node thread anim_loop_aligned(self, "wait_idle", undefined, "stopIdle"); // get him into the appropriate pose
...
//	self.combat_runanim = %mortar_loadguy_run;
	...
	if( !JUST_SETUP )
	{
		...
		// this should be the only anim_reach these guys do... anims should all match, and 
		// fewer anim_reach calls reduces popping
		node anim_reach(self, setupString[index]);
	}
	else
	{
		index = 0;
		node anim_reach(self, setupString[index]); // just to get the guy in the right position
	}
