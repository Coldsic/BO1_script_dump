...
track_players_ammo_count()
{
	...
	while(1)
	{
		...
		for(i=0;i<players.size;i++)
		{
			...
			if(!isDefined(weap) || weap == "none" || isSubStr( weap, "zombie_perk_bottle" ) || is_placeable_mine( weap ) || weap == "syrette_sp" || weap == "zombie_knuckle_crack" || weap == "zombie_bowie_flourish" || weap == "zombie_sickle_flourish" || issubstr( weap, "knife_ballistic_" ) || ( GetSubStr( weap, 0, 3) == "gl_" ) )
			{
				continue;
			}
...
initZombieLeaderboardData()
{
	...
	// DLC 2 Cod 5 Zombies Leaderboard Stats
	...
	// DLC 2 Cod 5 Zombie leaderboards
...
level._zombie_melee["zombie"][5] 				= %ai_zombie_attack_forward_v2;		// slow DOUBLE SWIPE
	...
	*/
	...
	// RAVEN BEGIN bhackbarth: temp faller anims
	...
	level._zombie_fall_anims["zombie"]["ai_zombie_jump_down_40"] = %ai_zombie_jump_down_40;
	level._zombie_fall_anims["zombie"]["ai_zombie_jump_down_72"] = %ai_zombie_jump_down_72;
	level._zombie_fall_anims["zombie"]["ai_zombie_jump_down_96"] = %ai_zombie_jump_down_96;
	level._zombie_fall_anims["zombie"]["ai_zombie_jump_down_120"] = %ai_zombie_jump_down_120;
	level._zombie_fall_anims["zombie"]["ai_zombie_jump_down_127"] = %ai_zombie_jump_down_127;
	level._zombie_fall_anims["zombie"]["ai_zombie_jump_down_184"] = %ai_zombie_jump_down_184;
	level._zombie_fall_anims["zombie"]["ai_zombie_jump_down_190"] = %ai_zombie_jump_down_190;
	// RAVEN END
...
player_out_of_playable_area_monitor()
{
	...
	while ( true )
	{
		if ( !self in_life_brush() && (self in_kill_brush() || !self in_playable_area()) )
		{
...
/#
				...
				if ( isdefined( self isinmovemode( "ufo", "noclip" ) ) )
				{
					wait( 3 );
					continue;
				}
...
{
	if ( "minigun_zm" == self GetCurrentWeapon() )
	{
...
round_spawning_fall_test()
{
	...
	while (true)
	{
		spawn_point = random(spawn_points);
...
/#
	...
	{
		level.round_spawn_func = ::round_spawning_fall_test;
	}
#/
