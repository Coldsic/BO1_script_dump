...
update_flameboard(par1, par2, par3, par4)
{
	//billboard_appear();
	update_billboard( par1, par2, par3, par4 );	
	//wait 3;
	//	billboard_fadeout();
}
...
destroy_on_notify(mynotify, notifyent)
{
	...
	while(1)
	{
		if ( player buttonpressed("BUTTON_RSHLDR") )
		{
			...
			while(1)
			{
				...
				if ( player attackbuttonpressed("DPAD_UP") )
				{
					spot.origin = spot.origin+(0,0,1);
					wait waittime;
				}
...
random_int_not_in_array(min, max, array)
{
	...
	while(1)
	{
		...
		if (counter == 200)
		{
			...
			AssertEx( !IsDefined(i), "All numbers in array have been used");
			return min;
		}
	}
}
...
shuffle_array(array)
{
	...
	for (i=0; i < array.size; i++)
	{
		...
		temparray = array_add(temparray, tempentry);
	}
	
	AssertEx(temparray.size == array.size , "The return array is not the same size as the given array");
	return temparray;
}
...
struct_spawn(targetname, spawn_func, param1, param2, param3, param4, param5)
{
	...
	{
		struct_spawners = [];
		struct_spawners = array_add(struct_spawners,targetname);
	...
	AssertEx(IsDefined(struct_spawners), "Spawner array not defineed in struct_spawn");
...
get_all_threatbias_groups()
{
	...
	for (i=0; i < ai.size; i++)
	{
		...
		{
			tb_groups = array_add(tb_groups, my_tb_group);
		}
	}
	tb_groups = array_add(tb_groups, get_players()[0] GetThreatBiasGroup() );
	return tb_groups;
}

set_generic_threatbias(num, group1, group2)
{
	AssertEx( (IsDefined(group1) && IsDefined(group2)) || (!IsDefined(group1) && !IsDefined(group2) ) , "Only one group defined.  Either pass in 2 groups or none at all");
