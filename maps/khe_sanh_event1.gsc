...
event1_medic_vignette()
{
	...
	medic_soldier_names = [];
	medic_soldier_names[0] = "medic_soldier_01";
	medic_soldier_names[1] = "medic_soldier_02";

	anim_node = GetEnt( "e1_medic_soldiers_align", "targetname" );
	wounded_spawn = GetStartOrigin( anim_node.origin, anim_node.angles, level.scr_anim["medic_soldier_03"]["intro"][0] );
	wounded_angles = GetStartAngles( anim_node.origin, anim_node.angles, level.scr_anim["medic_soldier_03"]["intro"][0] );

	medic_soldier_names[2] = spawn_fake_character(wounded_spawn, wounded_angles, "wounded_torso");
	medic_soldier_names[2].animname = "medic_soldier_03";
	medic_soldier_names[2] UseAnimTree(#animtree);
	medic_soldier_names[2] SetAnim( level.scr_anim["medic_soldier_03"]["intro"][0], 1, 0, 0 );

	level thread do_vignette("e1_medic_soldiers_align", medic_soldier_names, "intro", true, 1, "medic", false, true);

	wait(0.05);

	level.vignette_group["medic"][0] Attach("anim_jun_stretcher", "tag_inhand");

	// wait till jeep start
	flag_wait("jeep_ride_start");
...
event1_barechest_guy_vignette()
{
	...
	// barechest guy
	align_node1 = GetEnt("e1_barechest_align", "targetname");
	...
	// barechest guy 2
	align_node2 = GetEnt("e1_barechest_align_2", "targetname");
	...
	// delete
	barechest_guy1 delete();
	barechest_guy2 delete();
}
...
event1_bodybag_vignette_humans( align_node )
{
	...
	bodybag_guys[0] = spawn_fake_character(align_node.origin, align_node.angles, "chaplain");
	bodybag_guys[0].animname = "bodybags_chaplan";
...
//	wait(3.0);
	...
	align_node = GetEnt("e1_bodybags_align", "targetname");
