...
#include maps\mp\gametypes\_armor;
...
dev_cac_init()
{
	...
	for ( ;; )
	{
		...
		switch( GetDvar( #"devgui_dev_cac" ) )
		{
			...
			case "dpad_hat":
				host thread dev_cac_dpad_think( "hat", ::dev_cac_cycle_hat, "" );
				break;
...
dev_cac_cycle_hat( forward, tag )
{
	...
	player = level.dev_cac_player;
	keys = GetArrayKeys( level.cac_functions[ "set_hat_model" ] );

	if ( player.cac_hat_type == "none" )
		player.cac_hat_type = keys[0];
	else if ( forward )
		player.cac_hat_type = next_in_list( player.cac_hat_type, keys );
	else 
		player.cac_hat_type = prev_in_list( player.cac_hat_type, keys );
