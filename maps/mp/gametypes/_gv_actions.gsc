...
DoSideBets( rule )
{
	...
	game["side_bets"] = true;
	level.sidebet = true;
	if( isDefined( level.onStartWagerSidebets ) )
	{
		[[level.onStartWagerSidebets]]();
	}
	thread maps\mp\gametypes\_globallogic::endGame( "tie", "Going to side bets..." );
	level waittill ( "side_bet_begin" );
	players = level.placement["all"];
	level.sideBetPlayers = [];
	for( playerIndex = 0 ; playerIndex < players.size ; playerIndex++ )
	{
		if( isDefined( players[playerIndex] ) && ( !isDefined( players[playerIndex].pers["eliminated"] ) || !players[playerIndex].pers["eliminated"] ) )
		{
			level.sideBetPlayers[level.sideBetPlayers.size] = players[playerIndex];
			players[playerIndex].pers["sideBetCount"] = 0;
		}
	}
	level.sideBetPlayers = SortPlayersByLivesDescending( level.sideBetPlayers );
	for( playerIndex = 0 ; playerIndex < level.sideBetPlayers.size ; playerIndex++ )
	{
		level.sideBetPlayers[playerIndex].sideBetNumber = playerIndex + 1;
	}
	for( playerIndex = level.sideBetPlayers.size ; playerIndex < 3 ; playerIndex++ )
	{
	...
	for( playerIndex = 0 ; playerIndex < level.players.size ; playerIndex++ )
	{
		...
		player setClientDvars(
			"sidebet_p1", level.sideBetPlayers[0].name,
			"sidebet_p2", level.sideBetPlayers[1].name,
			"sidebet_p3", level.sideBetPlayers[2].name,
			"sidebet_p1_lives", level.sideBetPlayers[0].pers["lives"],
			"sidebet_p2_lives", level.sideBetPlayers[1].pers["lives"],
			"sidebet_p3_lives", level.sideBetPlayers[2].pers["lives"],
			"sidebet_endTime", level.sideBetEndTime
		);
		player notify ( "reset_outcome" );
		player openMenu( game["menu_wager_side_bet"] );
	}
	thread maps\mp\gametypes\_wager::SideBetTimer();
...
DoChangeClass( rule )
{
	...
	for( targetIndex = 0 ; targetIndex < targets.size ; targetIndex++ )
	{
		...
		target maps\mp\gametypes\_hardpoints::giveOwnedKillstreak();
...
GiveOrRemoveKillstreak( rule, shouldGive )
{
	if( !DoesRuleHaveValidParam( rule ) )
		return;
	killstreak = maps\mp\gametypes\_hardpoints::getKillstreakByMenuName( rule.params[ 0 ] );
	targets = GetTargetsForGameTypeEventRule( rule );
	for ( targetIndex = 0 ; targetIndex < targets.size ; targetIndex++ )
	{
		target = targets[ targetIndex ];
		if( shouldGive )
		{
			targets[ targetIndex ] maps\mp\gametypes\_hardpoints::giveKillstreak( killstreak );
		}
		else
		{
			weapon = maps\mp\gametypes\_hardpoints::getKillstreakWeapon( killstreak );
...
DoScaleReloadSpeed( rule )
{
	...
	for ( targetIndex = 0 ; targetIndex < targets.size ; targetIndex++ )
	{
		targets[ targetIndex ] setclientdvar( "ui_gv_reloadSpeedModifier", rule.params[0] );
	}
}