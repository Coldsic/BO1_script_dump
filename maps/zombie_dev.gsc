...
main()
{
	maps\zombie_dev_fx::main();
	maps\zombie_dev_amb::main();
	...
	level.zombie_anim_override = maps\zombie_dev::anim_override_func;
	...
	level.exit_level_func = ::zombie_dev_exit_level;
	...
	maps\_compass::setupMiniMap("menu_map_zombie_dev"); 
	level.ignore_spawner_func = ::zombie_dev_ignore_spawner;
	level.zone_manager_init_func = ::zombie_dev_zone_init;
...
zombie_dev_ignore_spawner( spawner )
{
	// no power, no quads
	if ( !flag("power_on") )
	{
...
zombie_dev_exit_level()
{
	zombies = GetAiArray( "axis" );
	for ( i = 0; i < zombies.size; i++ )
	{
		zombies[i] thread zombie_dev_find_exit_point();
	}
}
zombie_dev_find_exit_point()
{
	self endon( "death" );
	player = getplayers()[0];
	dist_zombie = 0;
